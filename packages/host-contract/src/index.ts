...
export interface BrowserLlmCompleteMessage {
  role: 'system' | 'user' | 'assistant';
  content: string;
}

export interface BrowserLlmCompletePayload {
...
export interface BrowserLlmCompleteResult {
  text: string;
  // Display-grade model name only; hosts must not leak routing internals.
  model?: string;
...
export interface BrowserPermissionsRequestPayload {
  ...
  // When true, revokes the current resource's session grants instead of requesting new ones.
  revoke?: boolean;
}

export interface BrowserPermissionsResult {
  granted: BrowserPermissionGrant[];
  expiresAt?: number;
}

export interface BrowserPermissionsConfirmation {
  actor: BrowserMetaAppBridgeActor;
  grants: BrowserPermissionGrant[];
  reason?: string;
}

export interface BrowserPermissionsConfirmRequest {
  resourceUri: string;
...
export interface BrowserTrustedActionInput extends BrowserActorInput {
  ...
  kind: BrowserTrustedActionKind;
  payload?: Record<string, unknown>;
  // Session-scoped id generated by the Browser page on load. Hosts may use it
  // to scope in-memory grants (e.g. protocol write grants) so they die with
  // the page session on refresh.
  sessionId?: string;
}
