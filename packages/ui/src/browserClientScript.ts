...
export function buildBrowserClientScript(input: BrowserClientScriptInput): string {
  ...
  const browserEndpoints = {
    ...
    metafileUpload: apiBasePath + '/metafile-upload'
  };
  ...
  const CUSTOM_BOT_PAGE_HELP = 'When enabled, Bot Pages can render the custom MetaApp or Metafile declared on /info/homepage. When disabled, Browser always uses the selected built-in template.';
  const state = {
    runtime: null,
  ...
  function htmlFrameSandbox(url) {
    ...
    const base = 'allow-scripts allow-downloads';
    const origin = window.location && window.location.origin ? window.location.origin : '';
    if (!origin) return base;
    try {
      const frameOrigin = new URL(url, window.location.href || undefined).origin;
  ...
  function extractPinId(value) {
    ...
    if (!text) return '';
    if (new RegExp('^metafile:\\/\\/', 'i').test(text)) text = text.slice('metafile://'.length);
    if (new RegExp('^pin:\\/\\/', 'i').test(text)) text = text.slice('pin://'.length);
    text = text.split(/[?#]/, 1)[0].replace(new RegExp('\\.[A-Za-z0-9]+$', 'u'), '').toLowerCase();
    return /^[0-9a-f]{64}i[0-9]+$/i.test(text) ? text : '';
  }
  function sanitizedActorSnapshot(currentActor) {
    const globalMetaId = textValue(currentActor && currentActor.globalMetaId);
    if (!globalMetaId) return null;
    const snapshot = {
      uri: 'metaid://' + globalMetaId,
      globalMetaId,
      name: textValue(currentActor && currentActor.label) || globalMetaId
    };
    const avatarPinId = extractPinId(currentActor && currentActor.avatarPinId);
  ...
  function validateMetafileUploadParams(params) {
    ...
    const value = { source: { kind: 'host-picker' } };
  ...
  async function handleBridgePinWrite(sourceWindow, id, params) {
    const validation = validatePinWriteParams(params);
    if (!validation.ok) {
      bridgePostMessage(sourceWindow, bridgeResponse(id, false, validation.error));
      return;
    }
    try {
      const payload = await api(endpointWithActor(browserEndpoints.actions), {
        method: 'POST',
      ...
      if (payload && payload.ok === false) {
        bridgePostMessage(sourceWindow, bridgeResponse(id, false, {
          code: textValue(payload.code) || 'pin_write_failed',
          message: textValue(payload.message) || 'MetaID PIN write failed.'
        }));
        return;
      }
      ...
      bridgePostMessage(sourceWindow, bridgeResponse(id, false, {
        code: textValue(error && error.code) || 'pin_write_failed',
        message: textValue(error && error.message) || 'MetaID PIN write failed.'
      }));
    }
  }
  async function handleBridgeMetafileUpload(sourceWindow, id, params) {
    const validation = validateMetafileUploadParams(params);
    if (!validation.ok) {
      bridgePostMessage(sourceWindow, bridgeResponse(id, false, validation.error));
      return;
    }
    try {
      const payload = await api(endpointWithActor(browserEndpoints.metafileUpload), {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
      ...
      if (payload && payload.ok === false) {
        bridgePostMessage(sourceWindow, bridgeResponse(id, false, {
          code: textValue(payload.code) || 'upload_failed',
          message: textValue(payload.message) || 'MetaFile upload failed.'
        }));
        return;
      }
      const result = payload && payload.data ? payload.data : payload;
      ...
      bridgePostMessage(sourceWindow, bridgeResponse(id, false, {
        code: textValue(error && error.code) || 'upload_failed',
        message: textValue(error && error.message) || 'MetaFile upload failed.'
      }));
    }
  }
  function handleBridgePrivateChatCompose(sourceWindow, id) {
    ...
    const owner = state.resource && state.resource.owner || {};
    const targetGlobalMetaId = textValue(owner.globalMetaId);
    if (features.privateChat !== true) {
      bridgePostMessage(sourceWindow, bridgeResponse(id, false, {
  ...
  function pinInspectorTxid(resource) {
  ...
  function pinInspectorPayloadIntro(resource) {
    ...
    const jsonPayload = pinInspectorParseJsonPayload(payload, rawPayload);
    const renderAsJson = contentType.includes('json') || (payload && typeof payload === 'object') || (jsonPayload && typeof jsonPayload === 'object');
  ...
  function pinInspectorRenderRawPayload(resource) {
    ...
    const payload = pinInspectorPayload(resource);
    const jsonPayload = pinInspectorParseJsonPayload(payload, rawPayload);
    const renderAsJson = pinInspectorContentType(resource).includes('json') || (payload && typeof payload === 'object') || (jsonPayload && typeof jsonPayload === 'object');
    if (renderAsJson) {
  ...
  function pinInspectorRenderMediaItems(resource) {
    ...
    const rawPayload = pinInspectorData(resource).rawPayload;
    const jsonPayload = pinInspectorParseJsonPayload(payload, rawPayload);
    const mediaSource = contentType.includes('json') || (payload && typeof payload === 'object') || (jsonPayload && typeof jsonPayload === 'object')
      ? jsonPayload
      : payload;
  ...
  function pinInspectorRenderRelatedLinks(resource) {
    ...
    const rawPayload = pinInspectorData(resource).rawPayload;
    const jsonPayload = pinInspectorParseJsonPayload(payload, rawPayload);
    const source = contentType.includes('json') || (payload && typeof payload === 'object') || (jsonPayload && typeof jsonPayload === 'object')
      ? jsonPayload
      : payload;
    const uris = new Set();
...
function openPinRawRecord(trigger) {
  ...
  const resource = state.resource;
  const rawRecord = resource ? pinInspectorRawPinRecord(resource) : null;
  if (!rawRecord) return false;
  openBrowserModal('Raw PIN record', '<pre class="browser-protocol-json">' + escapeHtml(JSON.stringify(rawRecord, null, 2)) + '</pre>');
  return true;
}
  function renderPinInspectorPage(resource) {
    ...
    const txid = pinInspectorTxid(resource);
    const contentType = pinInspectorContentType(resource);
  ...
  function resourceHtml(resource) {
    ...
    if (renderer.type === 'html-iframe') return '<iframe class="browser-html-frame" sandbox="' + htmlFrameSandbox(url) + '" src="' + escapeHtml(url) + '"></iframe>';
    if (renderer.type === 'pdf') return '<iframe class="browser-pdf" sandbox="allow-scripts allow-same-origin allow-downloads" src="' + escapeHtml(url) + '"></iframe>';
  ...
  function customBotPagesEnabled() {
    const value = settingValue('renderCustomBotPages');
    return value !== false;
  }
  ...
  function renderTemplateSettings() {
    ...
    return '<section class="browser-template-panel">' +
      ...
      '<section class="browser-template-builtins">' +
        ...
        '<div class="browser-template-options">' +
  ...
  async function loadBrowserSettingsData() {
    state.error = '';
    const settingsResponse = await fetch(browserSettingsEndpoint());
  ...
  async function saveBrowserSettings() {
    ...
    showBrowserLoadingModal('Browser Settings');
    try {
      const response = await fetch(browserSettingsEndpoint(), {
        method: 'PUT',
        headers: { 'content-type': 'application/json' },
  ...
  async function toggleCustomBotPages() {
    const nextValue = !customBotPagesEnabled();
    showBrowserLoadingModal('Browser Settings');
    try {
      const response = await fetch(browserSettingsEndpoint(), {
        method: 'PUT',
      ...
      const payload = await response.json();
      if (!payload.ok) throw new Error(payload.message || payload.code || 'Custom Bot Page setting update failed');
      state.settingsData = payload.data;
      renderBrowserSettings();
      const uri = state.resource && state.resource.uri || input && input.value || '';
      if (uri) await navigateTo(uri, { recordHistory: false });
    } catch (error) {
      state.error = error && error.message || 'Custom Bot Page setting update failed';
      renderBrowserSettings();
    }
  }
  ...
  document.addEventListener('click', (event) => {
    ...
    const customPagesButton = closestWithAttribute(target, 'data-browser-custom-pages-toggle');
    if (customPagesButton) {
      event.preventDefault();
      toggleCustomBotPages().catch(() => {});
      return;
    }
