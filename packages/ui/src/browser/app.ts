...
var PIN_INSPECTOR_PAGE_STYLE = '<style>' +
  ...
  '.browser-pin-markdown blockquote { margin: 0; padding: 8px 14px; border-left: 3px solid #cfe0ff; border-radius: 0 10px 10px 0; background: #f3f7ff; color: #33415c; }' +
  '.browser-pin-markdown ul, .browser-pin-markdown ol { margin: 0; padding-left: 22px; display: grid; gap: 4px; }' +
  '.browser-pin-markdown code { padding: 2px 6px; border-radius: 6px; background: #eef3f9; color: #24344e; font: 12px/1.5 ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace; overflow-wrap: anywhere; }' +
  '.browser-pin-markdown pre { margin: 0; padding: 14px 16px; border-radius: 10px; background: #182235; color: #d7e3f0; overflow-x: auto; }' +
  '.browser-pin-markdown pre code { padding: 0; background: none; color: inherit; }' +
  '.browser-pin-markdown hr { margin: 0; border: 0; border-top: 1px solid #d9e1ed; }' +
  '.browser-pin-markdown .browser-pin-md-image { display: block; margin: 4px 0; min-width: 0; }' +
  '.browser-pin-md-image-slot { display: block; min-width: 0; max-width: 100%; font-size: 13px; overflow-wrap: anywhere; }' +
  '.browser-pin-md-image-slot img { display: block; max-width: 100%; height: auto; max-height: 640px; object-fit: contain; border-radius: 10px; }' +
  '.browser-pin-md-image-slot.is-error { display: block; padding: 8px 12px; border: 1px solid #f0c8c4; border-radius: 10px; background: #fdecec; color: #b3261e; font-size: 12px; }' +
  ...
  'html[data-browser-resolved-theme="dark"] .browser-pin-markdown blockquote { border-left-color: #2f4a7a; background: #1b2638; color: #c2d0e6; }' +
  'html[data-browser-resolved-theme="dark"] .browser-pin-markdown code { background: #1b2638; color: #c2d0e6; }' +
  'html[data-browser-resolved-theme="dark"] .browser-pin-markdown pre { background: #0f1828; color: #c2d0e6; }' +
  'html[data-browser-resolved-theme="dark"] .browser-pin-markdown hr { border-top-color: #2b3850; }' +
  'html[data-browser-resolved-theme="dark"] .browser-pin-md-image-slot.is-error { border-color: #5a2c2c; background: #3a2022; color: #f0a0a0; }' +
...
// Inline markdown tokens, scanned in one pass over the raw text so bare
// MetaWeb URIs never get linkified inside markdown link/image hrefs or code
// spans (which the old escape-then-replace pipeline could not guarantee).
// MetaWeb URIs are ASCII-only: the body match stops at ASCII
// whitespace/control characters, quotes, brackets, and any non-ASCII
// character, so trailing non-ASCII prose (CJK punctuation, full-width
// parentheses) is never absorbed into the recognized URI.
var PIN_INSPECTOR_MD_INLINE_TOKEN_RE = /(\`[^\`]+\`)|(!\\[[^\\]]*\\]\\([^)\\s]+(?:\\s+"[^"]*")?\\))|(\\[[^\\]]+\\]\\([^)\\s]+(?:\\s+"[^"]*")?\\))|((?:metaid|metaapp|metafile|map|pin):\\/\\/[^\\x00-\\x20"'<>()\\[\\]{}\\u007f-\\uffff]+)/g;
var PIN_INSPECTOR_MD_IMAGE_RE = /^!\\[([^\\]]*)\\]\\(([^)\\s]+)(?:\\s+"([^"]*)")?\\)$/;
var PIN_INSPECTOR_MD_LINK_RE = /^\\[([^\\]]+)\\]\\(([^)\\s]+)(?:\\s+"([^"]*)")?\\)$/;

// Metafile images inside markdown reuse the pin inspector media hydration
// slots (data-browser-media-preview-ref/slot), so the client resolves them
// through the same metafile content gateway as Related Media.
function pinInspectorMarkdownImageHtml(href, alt, title) {
  var titleAttribute = title ? ' title="' + escapeHtml(title) + '"' : '';
  var label = textValue(alt) || href;
  return '<span class="browser-pin-md-image" data-browser-media-preview-ref="' + escapeHtml(href) + '"' + titleAttribute + '>' +
    '<span class="browser-pin-md-image-slot" data-browser-media-preview-slot>' +
    pinInspectorReferenceHtml(href, escapeHtml(label)) +
    '</span></span>';
}

function pinInspectorInlineTokenHtml(token) {
  var match = token.match(PIN_INSPECTOR_MD_IMAGE_RE);
  if (match) {
    var imageHref = textValue(match[2]);
    if (imageHref.indexOf('metafile://') === 0 && pinInspectorIsImageReference(imageHref)) {
      return pinInspectorMarkdownImageHtml(imageHref, match[1], match[3]);
    }
    return pinInspectorReferenceHtml(imageHref, escapeHtml(match[1] || imageHref), match[3]);
  }
  match = token.match(PIN_INSPECTOR_MD_LINK_RE);
  if (match) {
    return pinInspectorReferenceHtml(textValue(match[2]), escapeHtml(match[1] || match[2]), match[3]);
  }
  if (token.charAt(0) === '\`') {
    return '<code>' + escapeHtml(token.slice(1, -1)) + '</code>';
  }
  // Bare MetaWeb URI autolink, displayed in full; trailing punctuation stays
  // outside the link.
  var uri = token;
  var trailing = '';
  while (uri && '),.;!?'.indexOf(uri.charAt(uri.length - 1)) !== -1) {
    trailing = uri.charAt(uri.length - 1) + trailing;
    uri = uri.slice(0, -1);
  }
  var schemeIndex = uri.indexOf('://');
  if (uri && schemeIndex > 0 && uri.length > schemeIndex + 3) {
    return pinInspectorReferenceHtml(uri, escapeHtml(uri)) + escapeHtml(trailing);
  }
...
function pinInspectorInlineMarkdown(value) {
  ...
  var lastIndex = 0;
  var match;
  PIN_INSPECTOR_MD_INLINE_TOKEN_RE.lastIndex = 0;
  while ((match = PIN_INSPECTOR_MD_INLINE_TOKEN_RE.exec(input)) !== null) {
    output += escapeHtml(input.slice(lastIndex, match.index));
    output += pinInspectorInlineTokenHtml(match[0]);
...
function pinInspectorIsFenceLine(line) {
  return /^ {0,3}\`\`\`/.test(textValue(line));
}

// Splits markdown into fenced-code and text segments before blank-line block
// splitting, so blank lines inside a code fence cannot break it apart.
function pinInspectorSplitMarkdownFences(input) {
  var segments = [];
  ...
  for (var i = 0; i < lines.length; i++) {
    var line = lines[i];
    if (currentType === 'text' && pinInspectorIsFenceLine(line)) {
      if (current.length) segments.push({ type: 'text', lines: current });
      current = [line];
      currentType = 'fence';
      continue;
    }
    if (currentType === 'fence' && pinInspectorIsFenceLine(line)) {
      current.push(line);
      segments.push({ type: 'fence', lines: current });
      current = [];
      currentType = 'text';
      continue;
    }
  ...
  if (current.length) segments.push({ type: currentType, lines: current });
  return segments;
}

function pinInspectorRenderFence(lines) {
  var end = lines.length > 1 && pinInspectorIsFenceLine(lines[lines.length - 1]) ? lines.length - 1 : lines.length;
  var body = [];
  for (var i = 1; i < end; i++) body.push(textValue(lines[i]));
  return '<pre><code>' + escapeHtml(body.join('\\n')) + '</code></pre>';
}

function pinInspectorRenderMarkdownBlocks(value) {
  // Blocks split on blank lines may still carry trailing blank lines left over
  // from fence segmentation; drop them so every-line checks (blockquote,
  // lists) are not defeated by empty entries.
  return String(value || '').split(/\\n{2,}/).filter(Boolean).map(function (block) {
    ...
    if (lines.every(function (line) {
      return /^>/.test(textValue(line));
    })) {
      return '<blockquote>' + lines.map(function (line) {
        return pinInspectorInlineMarkdown(textValue(line).replace(/^>\\s?/, ''));
      }).join('<br>') + '</blockquote>';
    }
...
function pinInspectorRenderMarkdown(value) {
  ...
  return pinInspectorSplitMarkdownFences(input).map(function (segment) {
    if (segment.type === 'fence') return pinInspectorRenderFence(segment.lines);
    return pinInspectorRenderMarkdownBlocks(segment.lines.join('\\n'));
  }).join('');
}
...
// Many pin protocols wrap a markdown document inside a JSON payload: the JSON
// body carries a contentType/content-type field of text/markdown plus the
// document itself in a content field.
function pinInspectorMarkdownPayloadContent(jsonPayload) {
  var body = objectValue(jsonPayload);
  var contentType = textValue(body.contentType || body['content-type']).toLowerCase();
  if (contentType.indexOf('text/markdown') !== 0) return '';
  return typeof body.content === 'string' && textValue(body.content) ? body.content : '';
}

var PIN_INSPECTOR_MARKDOWN_IN_JSON_INTRO = 'JSON payload carries a text/markdown content field; rendered as document content.';
...
function pinInspectorPayloadIntro(current) {
  ...
  if (renderAsJson && pinInspectorMarkdownPayloadContent(jsonPayload)) return PIN_INSPECTOR_MARKDOWN_IN_JSON_INTRO;
...
function pinInspectorRenderPayload(current) {
  ...
  var jsonPayload = pinInspectorParseJsonPayload(payload, rawPayload);
  var renderAsJson = contentType.indexOf('json') !== -1 || (payload && typeof payload === 'object') || (jsonPayload && typeof jsonPayload === 'object');
  var markdownContent = renderAsJson ? pinInspectorMarkdownPayloadContent(jsonPayload) : '';
  if (renderAsJson && markdownContent) {
    return '<div class="browser-pin-markdown">' + pinInspectorRenderMarkdown(markdownContent) + '</div>';
  }
  if (renderAsJson) {
...
function pinInspectorIsBrowserUriTerminator(char) {
  ...
  // MetaWeb URIs are ASCII-only: ASCII whitespace/control characters and any
  // non-ASCII character (CJK prose, full-width punctuation) end the URI.
...
var LOOPBACK_HTTP_ORIGIN_RE = /^https?:\\/\\/(?:127\\.0\\.0\\.1|localhost|\\[::1\\])(?::\\d+)?$/i;
var PREVIEW_ASSETS_PATH_PREFIX = '/api/browser/preview-assets/';

// MetaApp renderer URLs from a standalone host point at its ephemeral loopback
// preview origin (http://127.0.0.1:<port>), which exists only on the machine
// running the Browser server. Loopback-hosted pages (local dev, embedded
// hosts) reach it fine, but on a publicly hosted page every visitor's browser
// would dial its own loopback and get "connection refused". When the page
// itself is served from a non-loopback http(s) origin, rewrite those preview
// URLs onto the page origin (the main server exposes the same preview-assets
// routes); the frame then renders same-origin and htmlFrameSandbox drops
// allow-same-origin, so the app stays opaque and cannot touch the page.
function rendererUrl(rawValue) {
  var url = safeUrl(rawValue);
  if (!url || url.charAt(0) === '/') return url;
  var origin = window.location && window.location.origin ? String(window.location.origin) : '';
  if (!origin || !/^https?:/i.test(origin) || LOOPBACK_HTTP_ORIGIN_RE.test(origin)) return url;
  try {
    var parsed = new URL(url);
    if (!LOOPBACK_HTTP_ORIGIN_RE.test(parsed.origin)) return url;
    if (parsed.pathname.indexOf(PREVIEW_ASSETS_PATH_PREFIX) !== 0) return url;
...
function htmlFrameSandbox(url) {
  // Opaque-origin frames cannot export canvases (Chromium taints every image
  // they draw, so canvas.toBlob throws) and cannot trigger downloads without
  // the allow-downloads token. Keep the frame's own real origin — which is
  // safe only because it is NOT the Browser page origin, so the app can never
  // script the UI or call its same-origin API — plus downloads. Frames served
  // same-origin with the page stay fully opaque (no allow-same-origin).
  var base = 'allow-scripts allow-downloads';
  var origin = window.location && window.location.origin ? window.location.origin : '';
  if (!origin) return base;
  try {
    var frameOrigin = new URL(url, window.location.href || undefined).origin;
    return frameOrigin === origin ? base : 'allow-scripts allow-same-origin allow-downloads';
  } catch (error) {
    return base;
  }
}
...
function renderRenderer(current) {
  ...
  var url = rendererUrl(renderer.url);
  if (type === 'bot-page') {
