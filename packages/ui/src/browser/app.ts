...
var PIN_INSPECTOR_MD_INLINE_TOKEN_RE = /(\`[^\`]+\`)|(!\\[[^\\]]*\\]\\([^)\\s]+(?:\\s+"[^"]*")?\\))|(\\[[^\\]]+\\]\\([^)\\s]+(?:\\s+"[^"]*")?\\))|((?:metaid|metaapp|metafile|map|pin):\\/\\/[^\\s"'<>()\\[\\]{}]+)/g;
...
function pinInspectorMarkdownImageHtml(href, alt, title) {
  var titleAttribute = title ? ' title="' + escapeHtml(title) + '"' : '';
  var label = textValue(alt) || pinInspectorShortReference(href);
...
function pinInspectorInlineTokenHtml(token) {
  ...
  // Bare MetaWeb URI autolink; trailing punctuation stays outside the link.
  var uri = token;
  var trailing = '';
  ...
  if (uri && schemeIndex > 0 && uri.length > schemeIndex + 3) {
    return pinInspectorReferenceHtml(uri, escapeHtml(pinInspectorShortReference(uri))) + escapeHtml(trailing);
  }
