...
// resolves them through the same metafile content gateway as Related Media.
...
markdownRenderer.use({
  extensions: [
    {
      // Autolinks bare MetaWeb URIs (pin://, metaid://, metaapp://, …) inside
      // markdown text. The start() hook keeps marked's inlineText tokenizer
      // from swallowing the URI before this extension sees it.
      name: 'metawebUri',
      level: 'inline',
      start(src: string) {
        const index = src.search(METAWEB_URI_PREFIX_PATTERN);
        return index < 0 ? undefined : index;
      },
      tokenizer(src: string) {
        const match = src.match(METAWEB_URI_PATTERN);
  ...
  renderer: {
    ...
    image({ href, title, text: alt }) {
      const normalizedHref = text(href);
      if (normalizedHref.startsWith('metafile://') && isImageReference(normalizedHref)) {
        return renderMarkdownMetafileImage(normalizedHref, alt, title);
      }
...
function markdownPayloadContent(jsonPayload: unknown): string {
  const body = record(jsonPayload);
  const contentType = text(body.contentType ?? body['content-type']).toLowerCase();
  if (!contentType.startsWith('text/markdown')) return '';
  return typeof body.content === 'string' && body.content.trim() ? body.content : '';
}

const MARKDOWN_IN_JSON_INTRO = 'JSON payload carries a text/markdown content field; rendered as document content.';
...
function payloadIntro(resource: BrowserResourceEnvelope): string {
  ...
  if (renderAsJson && markdownPayloadContent(jsonPayload)) {
    return MARKDOWN_IN_JSON_INTRO;
  }
  if (renderAsJson) {
...
function renderPayload(resource: BrowserResourceEnvelope): string {
  ...
  const renderAsJson = contentType.includes('json')
    || (payload && typeof payload === 'object')
    || (jsonPayload && typeof jsonPayload === 'object');
  const markdownContent = renderAsJson ? markdownPayloadContent(jsonPayload) : '';

  if (renderAsJson && markdownContent) {
    return `<div class="browser-pin-markdown">${renderMarkdown(markdownContent)}</div>`;
  }
  if (renderAsJson) {
