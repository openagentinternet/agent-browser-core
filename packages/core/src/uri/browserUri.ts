...
function hasValidGlobalMetaIdPayloadLength(versionChar: string, bytes: number[]): boolean {
  ...
  if (versionChar === 'y') {
    ...
    const m = bytes[0];
    const n = bytes[1];
    if (m === 0 || n === 0 || m > n || n > 15) return false;
    return bytes.length === 2 + n * 33;
  }
  return false;
}

function looksLikeBareGlobalMetaId(input: string): boolean {
  return /^id[qpzryt]1/i.test(input);
}

function normalizeBareGlobalMetaId(input: string): string | null {
  const normalized = input.toLowerCase();
  if (!looksLikeBareGlobalMetaId(normalized)) return null;
  if (normalized[3] !== '1') return null;
  const versionChar = normalized[2];
  if (!GLOBAL_META_ID_VERSION_CHARS.has(versionChar)) return null;

  const payload = normalized.slice(4);
  if (payload.length <= GLOBAL_META_ID_CHECKSUM_LENGTH) return null;
