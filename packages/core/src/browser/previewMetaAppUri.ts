...
export function parsePreviewMetaAppUri(input: string): ParsedPreviewMetaAppUri {
  ...
  // url.pathname retains percent-encoding; decode it, then collapse stray duplicate slashes.
  let decodedPath: string;
  try {
    decodedPath = decodeURIComponent(url.pathname);
  } catch {
    throw new Error('preview-metaapp URI path contains an invalid percent-escape.');
  }
  const normalizedPath = collapseSlashes(decodedPath);
