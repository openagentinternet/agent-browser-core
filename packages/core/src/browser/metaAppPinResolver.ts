...
export interface ResolveMetaAppPinToRecordInput {
  ...
  createPreviewSession?: MetaAppPreviewSessionFactory;
  now?: () => number;
}
...
function resolveContentUrl(reference: string, metafileContentBaseUrl: string): string | undefined {
...
export async function resolveMetaAppPinToRecord(
  input: ResolveMetaAppPinToRecordInput,
): Promise<BrowserCommandResult<MetaAppGalleryRecord>> {
  const pinId = normalizeMetaAppPinId(input.pinId);
  if (!pinId) {
    return browserCommandFailed('invalid_browser_uri', 'metaapp:// requires a 64-hex pinId ending in i0.');
  }

  const fetchImpl = input.fetch ?? globalThis.fetch;
  if (!fetchImpl) {
    return browserCommandFailed('browser_resolve_failed', 'A fetch implementation is required to resolve MetaApp pins.');
  }
  ...
  try {
    ...
    if (!pinRecord) {
      return browserCommandFailed('browser_resolve_failed', 'MetaApp pin lookup returned an invalid response.');
    }
    ...
    if (protocolPath !== METAAPP_PROTOCOL_PATH) {
      return browserCommandFailed(
        ...
        { expectedPath: METAAPP_PROTOCOL_PATH, actualPath: protocolPath || null },
      );
    }
    ...
    if (!indexFile) {
      return browserCommandFailed('browser_resolve_failed', 'MetaApp indexFile must be a relative path inside the app package.');
    }
    if (!contentReference) {
      return browserCommandFailed('browser_resolve_failed', 'MetaApp protocol is missing a content reference.');
    }
    ...
    return browserCommandSuccess(buildRecord({
      pinId,
      manApiBaseUrl,
