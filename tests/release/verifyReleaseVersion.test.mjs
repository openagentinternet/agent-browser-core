...
test("accepts repo tag v0.5.0", async () => {
  const result = await verifyReleaseVersion({ tag: "v0.5.0", repoRoot });

  assert.deepEqual(result, { version: "0.5.0" });
});

test("rejects repo tag v0.2.0", async () => {
  await assert.rejects(
    () => verifyReleaseVersion({ tag: "v0.2.0", repoRoot }),
    /Tag version 0\.2\.0 does not match root package version 0\.5\.0/,
  );
});
...
test("rejects mismatched internal dependency pin", async () => {
  ...
  try {
    ...
    await assert.rejects(
      () => verifyReleaseVersion({ tag: "v0.5.0", repoRoot: fixtureRoot }),
      /@openagentinternet\/agent-browser-core depends on @openagentinternet\/agent-browser-host-contract@0\.2\.0, expected 0\.5\.0/,
    );
  } finally {
